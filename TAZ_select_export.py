...
for scenario in scenarios:
    for taz in tazs:
        for interval in intervals:
            selection = arcpy.SelectLayerByAttribute_management(scenario, 'NEW_SELECTION', '{0} {1}'.format(taz, interval))
            arcpy.AddField_management(selection, 'site', 'TEXT', field_length=16)
            with arcpy.da.UpdateCursor(selection, ['site']) as cursor:
                for row in cursor:
                    row[0] = taz
                    cursor.updateRow(row)
            arcpy.Dissolve_management(selection, os.path.join(export_folder, "{0}{1}{2}{3}.shp".format(scenario, '_', taz, interval[2:])), ['site'], '', 'SINGLE_PART' )
            # arcpy.FeatureClassToFeatureClass_conversion(selection, export_folder, "{0}{1}{2}{3}.shp".format(scenario, "_", taz, interval[2:]))
